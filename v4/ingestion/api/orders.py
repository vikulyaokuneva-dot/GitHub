"""Orders API raw loader.

Input: WBApiClient + RunContext.
Output: RawSourcePayload for orders source.
...
def load_orders(client: WBApiClient, run_context: RunContext) -> RawSourcePayload:
    ...
    response = client.get_json(ORDERS, params=params, allow_statuses={204: []})
    ...
    if response.ok and rows:
        ...
        warnings.append("orders source returned empty payload")
    else:
    ...
    return RawSourcePayload(source_name="orders", payload=payload, status=status)
