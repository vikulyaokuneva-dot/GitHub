...
def _classify_cost_bucket(record: NormalizedRealizationRecord) -> tuple[str | None, bool]:
    """Classify one realization record into exactly one cost bucket.

    Rules are intentionally compact and conservative:
    - direct event_type routing for logistics/storage,
    - keyword routing for commission/acquiring/pvz/penalties,
    - ambiguous deduction/other events go to other_costs.
    """

    event_type = str(record.event_type or "other").strip().lower()
    text = _label(record)

    if event_type == "sale" or event_type == "return":
        return None, False
...
def classify_realization_components(
    ...
    *,
    ...
    The function operates only on normalized realization records.
    It intentionally avoids KPI aggregation and keeps unknown events explicit.
    """
    ...
    for record in realization_records:
        ...
        if event_type == "sale":
            if amount is None:
                ...
                warnings.append(f"sale event without amount: {record.raw_ref or record.record_id or 'unknown'}")
            else:
                sale_values.append(float(amount))
                revenue_values.append(float(amount))
                payout_values.append(float(amount))
            continue

        if event_type == "return":
            if amount is None:
                payout_missing_count += 1
                warnings.append(f"return event without amount: {record.raw_ref or record.record_id or 'unknown'}")
            else:
                payout_values.append(float(amount))
            continue
        ...
        bucket_values[bucket].append(float(amount))
