"""Smoke runner for v4 delivery email preview.

Runs pipeline to obtain outputs payload, then builds email preview via delivery stage.
Does not perform SMTP sending.
"""
...
def run_smoke(
    *,
    ...
    preview_path = delivery.get("email_preview_path")
    if not preview_path:
        print("SMOKE FAILED: email_preview_path is missing")
        return 2

    root = Path(output_dir).resolve()
    target = Path(str(preview_path)).resolve()
    if not _is_within(root, target):
        print(f"SMOKE FAILED: email preview is outside output_dir: {target}")
        return 2
    if not target.exists():
        print(f"SMOKE FAILED: email preview does not exist: {target}")
        return 2

    print("SMOKE OK")
    print(f"mode={normalized_mode}")
    print(f"email_preview_path={target}")
    return 0


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="v4 delivery email preview smoke run")
