...
def _enforce_kpi_totals(pdf_markdown: str, facts: dict) -> str:
    ...
    try:
        ...
        has_report_buyouts = isinstance(report_totals, dict) and "buyouts" in report_totals
        orders = acc.get("orders")
        if has_report_buyouts:
...
def build_local_report_from_facts(report_date: str, facts: dict) -> dict:
    ...
    if finance_status_norm == "ok":
        markdown_lines.append(f"- Количество финансовых строк: {_fmt_int(rows_count)}")
    elif finance_message:
        markdown_lines.append(f"- {finance_message}")
    else:
    ...
    if finance_status_norm == "ok":
        ...
        email_lines.append(finance_message)
    else:
