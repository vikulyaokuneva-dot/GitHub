from __future__ import annotations

from pathlib import Path, PureWindowsPath
from typing import Any

try:
    from .config import PROJECT_ROOT, RUNS_DIR
except ImportError:
    from config import PROJECT_ROOT, RUNS_DIR


KEY_PATHS = (
    "ai_director/orchestrator.py",
    "ai_director/tasks/tasks.json",
    "src/openrouter_client.py",
    "report_v2/",
    "docs/testing.md",
)

INCLUDED_FILE_MAX_CHARS = 2500

SKIP_TOP_LEVEL_ENTRIES = {
    ".env",
    ".git",
    ".pytest_cache",
    ".venv",
    "__pycache__",
}


def collect_project_context(
    *,
    max_chars: int = 8000,
    include_paths: list[str] | None = None,
    include_file_max_chars: int = INCLUDED_FILE_MAX_CHARS,
    project_root: str | Path = PROJECT_ROOT,
    runs_dir: str | Path = RUNS_DIR,
    exclude_run_dir: str | Path | None = None,
) -> dict[str, Any]:
    root = Path(project_root)
    runs = Path(runs_dir)
    include_lines, included_files, missing_files, requested_paths = _include_paths(
        root,
        include_paths,
        max_file_chars=include_file_max_chars,
    )
    sections = [
        "# Project Context",
        "",
        "## Top-level entries",
        *_top_level_entries(root),
        "",
        "## Key paths",
        *_key_paths(root),
    ]
    if include_lines:
        sections.extend(
            [
                "",
                "## Requested include_paths",
                *include_lines,
            ]
        )
    sections.extend(
        [
            "",
            "## Last AI Director runs",
            *_last_run_dirs(runs, exclude_run_dir=Path(exclude_run_dir) if exclude_run_dir else None),
            "",
            "## README_AI_DIRECTOR.md excerpt",
            _readme_excerpt(root / "ai_director" / "README_AI_DIRECTOR.md"),
        ]
    )
    text = "\n".join(sections).strip() + "\n"
    text, truncated = _truncate(text, max_chars=max_chars)
    return {
        "included": bool(text.strip()),
        "chars": len(text),
        "max_chars": max_chars,
        "truncated": truncated,
        "include_paths": requested_paths,
        "included_files": included_files,
        "missing_files": missing_files,
        "text": text,
    }


def _top_level_entries(root: Path) -> list[str]:
    if not root.is_dir():
        return [f"- missing project root: {root}"]

    entries = [
        path
        for path in root.iterdir()
        if path.name not in SKIP_TOP_LEVEL_ENTRIES
    ]
    entries = sorted(entries, key=lambda path: (not path.is_dir(), path.name.lower()))
    lines: list[str] = []
    for path in entries[:80]:
        suffix = "/" if path.is_dir() else ""
        lines.append(f"- {path.name}{suffix}")
    if len(entries) > 80:
        lines.append(f"- ... {len(entries) - 80} more")
    return lines or ["- (empty)"]


def _key_paths(root: Path) -> list[str]:
    lines = []
    for relative in KEY_PATHS:
        target = root / relative.rstrip("/")
        kind = "dir" if relative.endswith("/") else "file"
        status = "present" if target.exists() else "missing"
        lines.append(f"- {relative}: {status} ({kind})")
    return lines


def _include_paths(
    root: Path,
    include_paths: list[str] | None,
    *,
    max_file_chars: int,
) -> tuple[list[str], list[dict[str, Any]], list[dict[str, Any]], list[str]]:
    requested_paths = _coerce_include_paths(include_paths)
    if not requested_paths:
        return [], [], [], []

    lines: list[str] = []
    included_files: list[dict[str, Any]] = []
    missing_files: list[dict[str, Any]] = []

    for requested_path in requested_paths:
        target, reason = _safe_include_target(root, requested_path)
        if target is None:
            lines.append(f"- {requested_path}: skipped ({reason})")
            missing_files.append({"path": requested_path, "reason": reason})
            continue

        display_path = _display_path(root, target, fallback=requested_path)
        if not target.exists():
            lines.append(f"- {display_path}: missing")
            missing_files.append({"path": display_path, "reason": "missing"})
            continue
        if not target.is_file():
            lines.append(f"- {display_path}: skipped (not_file)")
            missing_files.append({"path": display_path, "reason": "not_file"})
            continue

        text = target.read_text(encoding="utf-8-sig", errors="replace")
        snippet, truncated = _truncate_file(text, max_chars=max_file_chars)
        included_files.append(
            {
                "path": display_path,
                "chars": len(snippet),
                "original_chars": len(text),
                "max_chars": max_file_chars,
                "truncated": truncated,
            }
        )
        lines.extend(
            [
                f"### {display_path}",
                f"- chars: {len(snippet)} of {len(text)}",
                f"- truncated: {str(truncated).lower()}",
                "",
                f'<file path="{display_path}">',
                snippet.rstrip(),
                "</file>",
                "",
            ]
        )

    return lines, included_files, missing_files, requested_paths


def _coerce_include_paths(include_paths: list[str] | None) -> list[str]:
    if include_paths is None:
        return []

    paths = include_paths if isinstance(include_paths, list) else [str(include_paths)]
    result: list[str] = []
    for value in paths:
        path = str(value).strip().replace("\\", "/")
        if path:
            result.append(path)
    return result


def _safe_include_target(root: Path, requested_path: str) -> tuple[Path | None, str]:
    normalized = requested_path.replace("\\", "/")
    windows_path = PureWindowsPath(requested_path)
    if (
        Path(requested_path).is_absolute()
        or windows_path.is_absolute()
        or bool(windows_path.drive)
        or normalized.startswith("/")
    ):
        return None, "absolute_path"

    parts = [part for part in normalized.split("/") if part not in ("", ".")]
    if any(part == ".." for part in parts):
        return None, "parent_reference"

    target = root.joinpath(*parts)
    try:
        root_resolved = root.resolve()
        target_resolved = target.resolve(strict=False)
        target_resolved.relative_to(root_resolved)
    except (OSError, ValueError):
        return None, "outside_repository"

    return target_resolved, ""


def _display_path(root: Path, target: Path, *, fallback: str) -> str:
    try:
        return target.resolve(strict=False).relative_to(root.resolve()).as_posix()
    except (OSError, ValueError):
        return fallback


def _last_run_dirs(runs_dir: Path, *, exclude_run_dir: Path | None = None) -> list[str]:
    if not runs_dir.is_dir():
        return [f"- no runs directory: {runs_dir}"]

    excluded = exclude_run_dir.resolve() if exclude_run_dir else None
    run_dirs = []
    for path in runs_dir.iterdir():
        if not path.is_dir():
            continue
        if excluded and path.resolve() == excluded:
            continue
        run_dirs.append(path)

    latest = sorted(run_dirs, key=lambda path: path.stat().st_mtime, reverse=True)[:3]
    return [f"- {path.name}" for path in latest] or ["- (none)"]


def _readme_excerpt(path: Path, max_chars: int = 3500) -> str:
    if not path.is_file():
        return "(missing)"

    text = path.read_text(encoding="utf-8-sig").strip()
    if len(text) <= max_chars:
        return text
    return text[: max_chars - 16].rstrip() + "\n...[truncated]"


def _truncate_file(text: str, *, max_chars: int) -> tuple[str, bool]:
    if max_chars <= 0:
        return "", bool(text)
    if len(text) <= max_chars:
        return text, False
    return text[:max_chars], True


def _truncate(text: str, *, max_chars: int) -> tuple[str, bool]:
    if max_chars <= 0:
        return "", bool(text)
    if len(text) <= max_chars:
        return text, False
    suffix = "\n...[context truncated]\n"
    keep = max(0, max_chars - len(suffix))
    return text[:keep].rstrip() + suffix, True
