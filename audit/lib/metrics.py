# audit/lib/metrics.py
import datetime as dt
import json
import re
from typing import Any, Dict, List, Tuple

from audit.lib.cogs import calc_cogs_for_rows
    ...
    *,
    ...
    for r in rows:
        ...
        if is_logistics:
            ...
            row_return_qty = i(r.get("return_qty") or 0)
            if row_return_qty > 0:
                returns_qty += row_return_qty
                if sku_i:
                    m = sku_get(sku_i)
                    m["returns_qty"] += float(row_return_qty)
            row_delivery_qty = i(r.get("delivery_qty") or 0)
            if row_delivery_qty > 0:
                delivery_qty += row_delivery_qty
    ...
    if revenue_override is not None and revenue_override > 0:
        gross_revenue = revenue_override
        tax = gross_revenue * float(tax_rate or 0.0)
        profit = gross_revenue - commission - logistics - storage - penalties - tax - cogs_total
        margin = safe_div(profit, gross_revenue)
    ...
    revenue_scale = (gross_revenue / total_sku_sales_revenue) if total_sku_sales_revenue > 0 and gross_revenue > 0 else 1.0
    if revenue_override is not None and revenue_override > 0:
        revenue_scale = (revenue_override / total_sku_sales_revenue) if total_sku_sales_revenue > 0 else 1.0
    ...
    for sku_i, m in sku_map.items():
        ...
        if revenue_override is not None and revenue_override > 0 and total_sku_sales_revenue > 0:
            net_rev = net_rev * revenue_scale
    ...
    top_sku_by_profit = sorted(
        [x for x in sku_items if int(x.get("sales_qty", 0) or 0) > 0],
    ...
    return {
        ...
        "return_rate": round(float(returns_qty) / float(delivery_qty), 4) if delivery_qty > 0 else 0.0,
