...
def render_email_text(payload: dict) -> str:
    ...
    lines = [
        ...
        *[f"{label}: {value}" for label, value in profit_assortment_rows],
        "",
        "Finance:",
        f"Realized sales: {_format_count(finance.get('realized_sales_qty'))} / {_format_money(finance.get('realized_sales_revenue'))}",
        f"Gross revenue: {_format_money(finance.get('gross_revenue'))}",
        f"Seller payout: {_format_money(finance.get('seller_payout'))}",
        f"Returns: {_format_count(finance.get('returns_qty'))}",
        f"Deliveries: {_format_count(finance.get('deliveries_qty'))}",
        f"WB commission: {_format_money(finance.get('wb_commission'))}",
        f"Logistics: {_format_money(finance.get('logistics_amount') if finance.get('logistics_amount') is not None else finance.get('logistics'))}",
